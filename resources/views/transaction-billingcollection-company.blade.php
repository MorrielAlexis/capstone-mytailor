				...
				<div id="shoppingCart" class="card-panel">
					<div class="card-content">
						...
						<div class="col s12" style="margin-top:20px" hidden>
							...
							<a href="{{URL::to('/transaction/payment/company/home')}}" class="left btn" style="background-color:teal; color:white; margin-left:10px">Go to Payment</a> -->
					...
					<div class="card-content">
						...
						<div class="col s12">
							...
							<table class="table centered data-company" align = "center" border = "1">
								...
								<tbody>
									...
									@foreach($cust as $custs)
										@if($customer->boolIsActive == 1 AND $customer->strJO_CustomerCompanyFK == $custs->strCompanyID)

									@if($customer->strPaymentStatus == "Pending")
									<tr style="background-color:rgba(54, 162, 235, 0.2)" @if($customer->strTransactionFK != $custs->strJobOrderID AND $customer->strJO_CustomerCompanyFK == $custs->strCompanyID) hidden @endif>
										...
										<td class="center" >{{ $customer->strCompanyName }}</td>
									...
									@elseif($customer->strPaymentStatus != "Pending")
									<tr @if($customer->strTransactionFK != $custs->strJobOrderID) hidden @endif>
							...
							<div class="col s12" style="margin-top:6%">
								<div class="divider"></div>
								<a href="{{URL::to('/transaction/payment/company/home')}}" class="right  btn" style="background-color:teal; color:white; margin-top:2%">Go to Payment</a>
							</div>
							...
							<div id="view{{ $customer->strPaymentID }}" class="modal modal-fixed-footer" style="width:70%; height:80%">
								<center><h5 style="color:teal">Order and Payment Information of Customer</h5></center>
									...
									<div class="modal-content col s12" style="padding:3%">
										@foreach($customers as $customer)
												@foreach($cust as $custs)
													@if($customer->boolIsActive == 1 AND $customer->strJO_CustomerCompanyFK == $custs->strCompanyID AND $customer->strTransactionFK == $custs->strJobOrderID)<div class="col s7">
											          	<div class="right col s8"><b><input id="customer-name" type="text" class="validate" value="{{ $customer->strCompanyName }}" readonly></b></div>
											          	<div class="left col s4" style="margin-top: 4%; color:teal">Company Name</div>	          
							      					</div>
							      					<div class="col s5" style="margin-left: 0">
											          <div class="right col s8"><b><input id="customer-id" type="text" class="validate" value="{{ $customer->strCompanyID }}" readonly></b></div>
											          <div class="left col s4" style="margin-top: 4%; color:teal">Company ID</div>
							      					</div>
											...
											<div class="modal-footer col s12" style="margin-top: 5%">
								                ...
								                <a class="waves-effect waves-green btn-flat" href="{{ URL::to('/transaction/collection/company/payment-records') }}"><font color="black">OK</font></a>
								            </div>
