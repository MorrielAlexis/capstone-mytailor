      ...
      <div class="row" style="margin:40px;">
        ...
        <div id="tabPockets" class="col s12 white" style="padding:20px; border: 2px teal accent-4;">
          ...
          <div class="col s12" style="padding:20px;"><h4>Pants' Pockets</h4></div>
              @foreach($pocketSegment as $pocketSegment)
              <div class="col s12">
              @foreach($pattern as $pattern)
                <div class="col s2" @if($pattern->strSegPStyleCategoryFK != $pocketSegment->strSegStyleCatID) hidden @endif>
                  <img class="materialboxed responsive-img" src="{{ URL::asset($pattern->strSegPImage)}}">
                  <p>
                    <input name="rdb_pattern{{ $pocketSegment->strSegStyleCatID }}" type="radio" class="filled-in" value = "{{ $pattern->strSegPatternID }}" id="{{ $pattern->strSegPatternID }}" />
              ...
              <div class="col s12">
                <div><button class="right btn-flat teal accent-4 white-text" type="type="submit"">Next step</button></div>
                <div><a class="left btn-flat teal accent-4 white-text" href="{{URL::to('customize-pants-style-pleats')}}">Previous step</a></div>
              </div>
